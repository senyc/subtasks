...
async function getCompletedTasks(projectId: number): Promise<Task[]> {
  const res = await fetch(`http://localhost:8000/project/${projectId}/tasks/completed`);
  if (!res.ok) {
...
export default (projectId: number,) =>
  useQuery({ queryKey: ["completedTasks", projectId], queryFn: () => getCompletedTasks(projectId) });
