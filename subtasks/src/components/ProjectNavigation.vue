<template>
  <section class="flex flex-row pb-3 pt-1 items-center gap-5 mr-0.5">
    ...
    <button class="cursor-pointer" @click="deleteCheckedProjects"><svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
...
const checkedProjects = inject("checked", [])
...
async function deleteCheckedProjects() {
  await Promise.all(checkedProjects.map(id => deleteProject(id)))
